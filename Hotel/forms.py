...
class HotelForm(TailwindFormMixin, forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'address': forms.Textarea(attrs={'rows': 2, 'placeholder': 'Enter full hotel address...'}),
            'description': forms.Textarea(attrs={'rows': 4, 'placeholder': 'Describe your hotel...'}),
            'slug': forms.TextInput(attrs={'placeholder': 'hotel-name-example'}),
        }
...
class HotelImageForm(TailwindFormMixin, forms.ModelForm):
    class Meta:
        ...
        help_texts = {
            'is_primary': 'Check this if it is the main cover photo.'
        }
...
class RoomCategoryForm(TailwindFormMixin, forms.ModelForm):
    class Meta:
        model = RoomCategory
        fields = ['hotel', 'title', 'base_price', 'capacity_adults', 'capacity_child', 'description', 'features']
        widgets = {
...
class RoomForm(TailwindFormMixin, forms.ModelForm):
    class Meta:
        model = Room
        fields = ['category', 'room_number', 'room_type', 'price_override', 'is_available']
        help_texts = {
